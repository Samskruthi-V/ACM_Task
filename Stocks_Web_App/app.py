...
def index():
    ...
    stocks = db.execute("SELECT symbol, SUM(shares) as shares, operation FROM purchase WHERE userID = ? GROUP BY symbol HAVING (SUM(shares)) > 0;",
                        session["user_id"])
    total_cash_stocks = 0
    for stock in stocks:
...
def buy():
    ...
    if request.method == "POST":
        ...
        if user_cash < (shares_price):
            ...
            db.execute("INSERT INTO purchase (userID, symbol, shares, price, operation) VALUES (?, ?, ?, ?, ?)",
...
def history():
    ...
    stocks = db.execute("SELECT * FROM purchase WHERE userID = ?", session["user_id"])
...
def sell():
    ...
    if request.method == "POST":
        ...
        stocks = db.execute(
            "SELECT SUM(shares) as shares FROM purchase WHERE userID = ? AND symbol = ?;",
            session["user_id"],
            symbol,
        )[0]
        ...
        stocks = db.execute("SELECT symbol FROM purchase WHERE userID = ? GROUP BY symbol",
                            session["user_id"])
